...
type CallbackHandlerV80 struct {
	events chan gomultistripe.CallbackEvent
	mu     sync.Once
}

func NewCallbackHandlerV80() *CallbackHandlerV80 {
	return &CallbackHandlerV80{
		events: make(chan gomultistripe.CallbackEvent, 100),
	}
}

func (h *CallbackHandlerV80) Events() <-chan gomultistripe.CallbackEvent {
	return h.events
}

func (h *CallbackHandlerV80) HandleWebhook(payload []byte, sigHeader string) error {
